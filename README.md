...
말로만 하면 감이 안 오니, **예시 산출물 한 세트**를 그대로 뒀습니다 — `사용자 잔액 출금` 기능을 네 단계로 통과시킨 결과예요.

| 단계 | 문서 | 여기서 볼 것 |
|---|---|---|
| **① 기획** | [요구사항](docs/example/2026-07-15-잔액-출금/잔액-출금-requirements.md) | 맨 위 **"여기서 잠그는 의도"** — 이 한 문장이 잠기는 지점 |
| **② 설계** | [개발방향](docs/example/2026-07-15-잔액-출금/잔액-출금-tech-design.md) | **대안 비교표** — 왜 그 선택인지 근거가 남음 |
| **③ 계획** | [구현계획서](docs/example/2026-07-15-잔액-출금/잔액-출금-implementation-plan.md) | task별 **원본 → 수정 후** 코드 + `# ⚠️ RISK` 주석 |
| **④ 실행** | 같은 구현계획서 하단 | **변경이력** — 실행이 남긴 "왜 · 언제 · 무엇이" |

기획자가 읽는 **`.html` 사본**도 같이 있어요 → [잔액-출금-requirements.html](docs/example/2026-07-15-잔액-출금/잔액-출금-requirements.html) *(내려받아 열면 그대로 렌더됩니다)*

각 문서 하단 **변경이력**을 보면 게이트에서 나온 질문이 어떻게 의도를 바꿨는지 추적됩니다 — 예: *"취소하면 돈은 어떻게 되나?"* → `FR-3` 수정.

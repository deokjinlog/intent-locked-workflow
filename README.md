...
11. **AskUserQuestion 한국어화** *(v1.1.10)* — 모든 게이트의 question 텍스트 + choices label 을 한국어로 번역. `value` 는 영어 enum 그대로 (라우팅 일관성). `yes/fix` → `예/수정 필요`, `PRD/Socratic` → `PRD/소크라테스식`, `Inline/Subagent` → `인라인/서브에이전트`, `merge/pr/keep/discard` → `머지/PR 생성/그대로 두기/폐기`.

...
- **Claude 메모리 폴더 자동 심링크**: 메인 레포의 `~/.claude/projects/<encoded>/memory` 를 워크트리의 동일 위치에 심링크. 워크트리 첫 세션부터 메인의 user/feedback/project 메모리 즉시 활용. 메인에 메모리 0건이거나 워크트리에 이미 메모리 폴더가 있으면 자동 생략.

---
description: "사용자가 /list-skills 슬래시를 명시 호출했을 때만 발동. 현재 쓸 수 있는 skill 을 세 그룹(내가 만든 것 / intent-locked-workflow 내장 / 기타)으로 묶어 전부 조회. 읽기 전용."
argument-hint: "(인자 없음)"
---
...
|---|---|---|
| A. 내가 만든 skill | `/new-skill` 로 만든 것 (출처 표식 `.intent-locked-workflow-skill.json` 보유) | ✅ 가능 |
| B. intent-locked-workflow 내장 skill | 플러그인에 원래 들어 있는 skill | ❌ 불가 (플러그인 소속) |
...
- **프로젝트**: `<cwd>/.claude/skills/*/` 중 `.intent-locked-workflow-skill.json` 존재하는 것
- **글로벌**: `~/.claude/skills/*/` 중 `.intent-locked-workflow-skill.json` 존재하는 것
...
- **다른 플러그인**: `~/.claude/plugins/cache/*/*/skills/*/` 중 intent-locked-workflow 제외 → 플러그인 이름으로 묶기
...
4. (선택) 그룹 A 는 `.intent-locked-workflow-skill.json` 의 `created` 값으로 생성 시각 표시 가능
...
■ intent-locked-workflow 내장 skill (N개)
...
- **출처 표식은 신뢰 신호일 뿐 보안 경계 아님** — 사용자가 표식 파일을 수동 복사하면 비-intent-locked-workflow skill 도 그룹 A 에 뜰 수 있음 (낮은 빈도, 수용).

...
## 고도화 품질 레이어 (모든 스타일 공통) ⭐

정적 시안이 아니라 **"살아있는 제품"** 으로. 각 시안 생성 시 아래를 **스타일 언어에 맞게** 반드시 포함한다 (기법은 공통, 표현은 스타일 따라 — SF HUD 는 스캔라인·글로우 점등, 럭셔리 다크는 느리고 우아하게, 브루탈리즘은 하드한 등장).

1. **로드 시퀀스 (히어로 연출, ~1.2초 이내)** — 요소 순차 등장(스태거) + 핵심 수치 **카운트업**(0→목표, JS ease-out). 첫인상을 만든다.
2. **깊이 3층** — L0 배경 / L1 표면(카드) / L2 부유(hover 시 떠오름 + 그림자에 **컬러 틴트**). 깊이 변화엔 색이 동반된다.
3. **마이크로 인터랙션 3개+** (각 150~250ms) — hover 반응 / "살아있는" 인디케이터(펄스·숨쉬기) / 행·막대 hover 강조. **"반응했다" 수준에서 멈춤** (과함 금지).
4. **타이포 위계 극단화** — 최대 텍스트(핵심 수치) vs 최소(캡션) 크기비 **4배+**.
5. **자체 크리틱 루프 (출력 전 필수)** — 스스로 심사위원이 되어: ①로드 후 3초간 시선 머무는 곳 있나 ②가장 밋밋한 1곳 → 디테일 추가 ③가장 과한 1곳 → 제거 ④팔레트·타이밍이 지시와 일치하나. **품질 편차를 줄이는 핵심 장치.**
6. **안전장치** — `prefers-reduced-motion` 시 애니메이션 제거 + 최종상태 즉시 렌더 / **JS 실패해도 최종 데이터가 HTML 에 존재**(JS 는 연출만 담당) / hover 는 `focus-visible` 에도 동일 적용.

> **저장된 프롬프트 키트**: `prompt-kit.md` (PART A 마스터 + PART B 12 스타일 블록). 사용자가 **도메인만** 주면 마스터 `{{DOMAIN}}` 을 채우고 12 블록과 조합해 생성. STYLE 03 트렌디 SaaS 가 **v2 고도화 표준 예시**.
...
- 각 agent 프롬프트 = **`prompt-kit.md` 의 [PART A 마스터(`{{DOMAIN}}`←사용자 도메인으로 채움)] + [PART B 배정 STYLE 블록 1개] + [고도화 품질 레이어]**. → 저장된 프롬프트 그대로 구현.
- 예시 스크립트:

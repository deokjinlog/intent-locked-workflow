...
3배치가 끝나면(또는 생성분에 대해) **`gallery.html`** 을 출력 디렉토리에 자동 생성한다:

- 번호 + 스타일명 카드 그리드(2~3열), 각 카드에 `<iframe src="style-NN-*.html">` 미리보기
- 카드 클릭 → 새 탭 확대(`target=_blank`) / 2개 선택 시 나란히 비교(선택 기능)
- self-contained 인라인 CSS, 다크 배경, 반응형

경로를 안내하고 "몇 번이 맘에 드세요? (1~2개)" 로 4단계 피드백을 받는다.

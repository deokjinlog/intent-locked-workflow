...
v0.9.0 에서 브레인스토밍~실행 워크플로우의 "얇은 명령 wrapper" 8개를 제거하고 스킬 단일 진입으로 전환. 원조 superpowers 철학(스킬 중심)에 정렬. 근거: 스킬이 이미 슬래시 호출 + 자동 발동 둘 다 하므로 wrapper 명령은 기능상 불필요 (유일한 값은 `/dj-superkit:이름` 접두사 표시뿐).
